...
class Confound(Regressor):
    ...
    def __init__(self, name, fitter, confounds):
        ...
        Regressor.__init__(self,name, fitter)
...
class Event(Regressor):
        ...
        if type(self.basis_set) is not str:
            ...
            if self.basis_set in ['fir', 'dct']:
                ...
                if self.n_regressors > (length_interval / self.sample_duration):
                    warnings.warn('Number of regressors ({}) is larger than the number of timepoints in the interval ({}). This model can only be fit using regularized methods.'.format(
                        self.n_regressors,
    ...
    def create_design_matrix(self, oversample=1):
        ...
        columns = pd.MultiIndex.from_product(([self.name], self.covariates.columns, self.L.columns),
                                             names=['event type', 'covariate', 'regressor'])
